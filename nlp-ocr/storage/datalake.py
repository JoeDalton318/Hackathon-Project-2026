...
Data Lake MinIO — 3 zones :

  raw/    → document original + extraction OCR (JSON)
  clean/  → texte OCR brut
  curated/ → données validées (Maria)
"""
...
def store_curated(doc_id: str, result) -> Optional[str]:
    ...
    key  = _key(ZONE_CURATED, doc_id, "extraction", "json")
    return key if _put(key, data, "application/json", {
        "document-id": doc_id,
...
def store_all_zones(file_bytes, file_name, extraction_result):
    ...
    return {
        ...
        "curated":        store_curated(doc_id, extraction_result),
    }