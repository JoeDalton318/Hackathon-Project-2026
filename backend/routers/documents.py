...
async def get_document(document_id: str):
    record = await document_service.get_record(document_id)
    if record is None:
...
async def process_document(document_id: str):
    ...
    return APIResponse(data={"document_id": document_id, "status": DocumentStatus.PENDING})
...
async def delete_document(document_id: str):
    ...
    if record is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document introuvable")
    await document_service.delete_record(document_id)
